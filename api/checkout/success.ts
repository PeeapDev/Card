 ...
 * Handles successful merchant/developer checkout payments
 */
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    console.log('[Checkout Success] Callback received:', {
      method: req.method,
      reference,
      businessId,
      sessionId,
    ...
    if (redirect && typeof redirect === 'string' && redirect.startsWith('http')) {
      ...
      // Redirect to default success page
    ...
    console.log('[Checkout Success] Redirecting to:', redirectUrl.toString());
